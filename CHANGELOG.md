...
- **Added** application-level shared state in `src/state/`: `MissionProvider` owns the persisted mission state and the forecast draft (`lat`, `lon`, `name`, `startDate`, `days`); `useMissionActions` centralizes record-history-and-navigate so Home and the drawer share one code path; `missionContext` exposes the value and the `useMissionContext` hook
...
- **Added** unit and component coverage: `src/state/MissionProvider.test.tsx`, `src/state/ResultsDraftSync.test.tsx`, `src/components/MissionDrawer.test.tsx`, a rewritten `src/pages/Home.test.tsx` (panel absent, shared-draft wiring) and `src/App.test.tsx` (hamburger placement and ARIA, every close path, focus entry/trap/return, inert and body-scroll lock with cleanup, navigation/drawer mutual exclusion, caret open/closed state)
- **Added** Playwright coverage in `e2e/mission-drawer.spec.ts`: desktop and mobile drawer layout, header layout at 1280/800/390/320, the existing Install-button scenario, focus trap and return, Escape and backdrop closing, the mobile navigation caret, mutual exclusion, internal drawer scrolling, body-scroll lock, no horizontal overflow, route availability, and `@axe-core/playwright` scans on Home and a non-Home route with the drawer open; `e2e/header.spec.ts` updated for the new hamburger
