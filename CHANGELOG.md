...
- **Set** `retries` in `playwright.config.ts` to 1 locally / 2 on CI so an occasional slow browser-process launch under parallel load does not fail an otherwise-passing suite; a real failure still fails on every attempt
