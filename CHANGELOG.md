...
- **Added** Playwright coverage in `e2e/mission-drawer.spec.ts`: desktop and mobile drawer layout, header layout at 1280/800/390/320, the existing Install-button scenario, immediate and repeated forward/reverse focus trap and return, Escape and backdrop closing, browser-level Save / Select / Run / Rerun navigation, the mobile navigation caret, mutual exclusion, internal drawer scrolling, body-scroll lock, no horizontal overflow, hash-aware route availability on a real non-Home route, and `@axe-core/playwright` scans on Home and a non-Home route with the drawer open; `e2e/header.spec.ts` updated for the new hamburger
