...
export default function Results() {
  ...
  const averageScore = forecasts.length > 0 
    ? Math.round(forecasts.reduce((sum, f) => sum + f.biteScore0100, 0) / forecasts.length * 10) / 10
    : 0;
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="text-center">
          ...
          <p className="text-gray-600">Fetching weather data and calculating bite scores</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              className={`px-3 py-1 text-sm rounded ${!useFahrenheit ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
          ...
          <div className="flex items-center gap-2">
            <button
              className={`px-3 py-1 text-sm rounded ${!useMph ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
      ...
      {forecasts.length > 0 && (
        <div className="grid gap-6 sm:grid-cols-2 mb-8">
          ...
          <div className="card p-6">
            ...
            <div className="text-2xl font-bold text-green-600">{bestDay.biteScore0100}</div>
          </div>
        </div>
      )}
      ...
      <div className="mt-8 text-center text-sm text-gray-500">
        <p>
          Data sources: Open-Meteo weather API, astronomical calculations for lunar phase
        </p>
        <p className="mt-1">
          Forecasts are for entertainment purposes and should not be the sole factor in fishing decisions
        </p>
      </div>
    </div>
  );
